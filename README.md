...
3. Чтобы в БД появились таблицы, которые есть в репозитории, нужно применить миграции — `alembic upgrade head`
4. Для того чтобы сервер `FastAPI` запустился с новыми данными для подключения, нужно обновить сессию терминала (закрыть старый терминал и открыть новый) и запустить сервер
